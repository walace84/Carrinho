...
class CarrinhoController extends Controller
{
    ...
    {
        // busca na table pedido,pegando o status do pedido e id da table users
    ...
    public function adicionar()
    {
        ...
        // model produto pegando o id do produto
        $produto = Produto::find($idproduto);
        if( empty($produto->id) ) {
            // msg de erro temporária
            $req->session()->flash('mensagem-falha', 'Produto não encontrado em nossa loja!');
            return redirect()->route('carrinho.index');
        }
        // id do user logado
        $idusuario = Auth::id();
        // faz uma consulta verificando se existe um pedido reservado para este user
        $idpedido = Pedido::consultaId([
            'user_id' => $idusuario,
            'status'  => 'RE' // Reservada
            ]);
        // se não tiver um produto reservado cria um novo pedido
        if( empty($idpedido) ) {
        ...
        // model pepidoProduto para criar na nessa tabela
    ...
    public function remover()
    {
        ...
        // quando for true remove apenas um item quando for false remove todos os itens
        ...
        // passa essas informações em forma de objetos para $produto
        ...
        // caso queira remover apenas um item
        if( $remove_apenas_item ) {
            $where_produto['id'] = $produto->id;
        }
        // se não for deletar só um produtos o delete deleta todos
        ...
        // caso não tenha mais produtos nesse pedido, ele deleta o pedido
        if( !$check_pedido ) {
            Pedido::where([
